# Claude Code Project Instructions — winres-kotlin

## Project Overview

This is **winres-kotlin**, a clean-room Kotlin Multiplatform port of the upstream Rust crate [`winres`](https://crates.io/crates/winres).

The upstream Rust source is the read-only translation oracle. When porting begins, clone it into `tmp/winres/` (gitignored). **Never edit `tmp/`.** If upstream looks wrong, the bug is in the port or in your understanding of Rust, not in `tmp/`.
...
```kotlin
// port-lint: source <path-relative-to-tmp/winres>
